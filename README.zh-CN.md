...
| 3 | OpenAI Codex CLI | `codex` / `codex` | 101,551 | `brew install --cask codex` | 只读沙箱和临时会话；部分宿主须让子进程脱离外层沙箱 | [文档](https://developers.openai.com/codex/cli/) |
...
|---|---|---|---|---|
| Claude Code | `claude` / `claude` | `brew install --cask claude-code` | 禁用工具且不保留会话；部分宿主须让子进程脱离外层沙箱 | [文档](https://code.claude.com/docs/en/overview) |
...
- 评审深度应与既定目标和当前场景相称：必改项必须有实质影响的证据，并采用足以解决问题的最小改动；面向假设未来的扩展性和额外机制只能作为可选建议。
...
- 在带外层沙箱的宿主中，Codex、Claude 和 Antigravity 都可能需要让整个子进程在宿主外层沙箱之外运行：Codex 需要正常状态目录和 app-server client，Claude 需要访问已配置的 API 端点、网络、认证与 Provider 设置，Antigravity 需要回环监听以及正常 app-data 和认证状态。此时子进程会获得当前用户级别的正常文件系统和网络访问权限。各调用方案仍保留内部限制——Codex 使用只读沙箱和临时会话，Claude 使用 Safe Mode、禁用工具且不保留会话，Antigravity 使用内部沙箱和空白工作目录——但这些限制不能等同于整个进程完全隔离；预览会披露完整权限范围。
